...
- O objetivo das análises é 1) identificar as evidências de validade e de precisão para aplicação do instrumento EIB em adolescentes de 14 anos que estão cursando o 9º ano escolar e 2) gerar as novas normas da EIB.
...
o que sobrou da reunião com Rod:

- ver var e significância da variância dos fatores de E na AFC; 

  R: É significativa.


- checar se a cor de org e lid ficam menor que 1 quando a fixação da métrica do fator vai para o item mais forte

  R: Não resolveu
  
- se fixar var do item não resolver, tentar fixar ~0.9 para cor entre lid e org e ver se o modelo funciona
  
  R: não resolveu

- checar tabela de normas com médias, sd e percentis com pop total (SP + MS), MS e SP e ver se existem difs significativas
  
  R: Maiores diferenças para Rotinas e Convencional


- juntar essas decisões naquele ppt e levar pra Gi
...
## Empreendedor - constrants
- Carga dos itens mais fortes fixadas em 1

- Correlação entre lideranca e organizacional fixada em 0.90
```{r }
# modelo
modelo_E <- '
lideranca =~ NA*EIB17 + EIB39 + EIB61_6 + EIB83_28 + 1*EIB105_50
organizacional =~ NA*EIB18 + EIB40 + EIB62_7 + EIB84_29 + 1*EIB106_51
politica =~ NA*EIB19 + EIB41 + EIB63_8 + EIB85_30 + 1*EIB107_52
vendas =~ NA*EIB20 + EIB42 + EIB64_9 + EIB86_31 + 1*EIB108_53

lideranca ~~ 0.9*organizacional
'
...
# Invariância 9º ano x EM {.tabset .tabset-fade}
...
#THRESHOLDS 
thresholds_escolaridade <-measEq.syntax(configural.model=modelo_I, data=dt_MT_SP_AFC,
                           ordered=TRUE, estimator="WLSMV",
                           group="banco", return.fit=TRUE,
                           group.equal="thresholds")
# summary(thresholds_escolaridade, fit.measures=T, standardized=T)
...
#METRICA
metrica_escolaridade <- measEq.syntax(configural.model=modelo_I, data=dt_MT_SP_AFC,
                         ordered=TRUE, estimator="WLSMV",
                         group="banco", return.fit=TRUE,
                         group.equal=c("thresholds","loadings"))
# summary(metrica_escolaridade, fit.measures=T, standardized=T)
...
#ESCALAR
escalar_escolaridade <- measEq.syntax(configural.model=modelo_I, data=dt_MT_SP_AFC,
                         ordered=TRUE, estimator="WLSMV",
                         group="banco", return.fit=TRUE,
                         group.equal=c("thresholds","loadings", 
                                       "intercepts"))
...
#THRESHOLDS 
thresholds_escolaridade <-measEq.syntax(configural.model=modelo_A, data=dt_MT_SP_AFC,
                           ordered=TRUE, estimator="WLSMV",
                           group="banco", return.fit=TRUE,
                           group.equal="thresholds")
# summary(thresholds_escolaridade, fit.measures=T, standardized=T)
...
#METRICA
metrica_escolaridade <- measEq.syntax(configural.model=modelo_A, data=dt_MT_SP_AFC,
                         ordered=TRUE, estimator="WLSMV",
                         group="banco", return.fit=TRUE,
                         group.equal=c("thresholds","loadings"))
# summary(metrica_escolaridade, fit.measures=T, standardized=T)
...
#ESCALAR
escalar_escolaridade <- measEq.syntax(configural.model=modelo_A, data=dt_MT_SP_AFC,
                         ordered=TRUE, estimator="WLSMV",
                         group="banco", return.fit=TRUE,
                         group.equal=c("thresholds","loadings", 
                                       "intercepts"))
...
#THRESHOLDS 
thresholds_escolaridade <-measEq.syntax(configural.model=modelo_S, data=dt_MT_SP_AFC,
                           ordered=TRUE, estimator="WLSMV",
                           group="banco", return.fit=TRUE,
                           group.equal="thresholds")
# summary(thresholds_escolaridade, fit.measures=T, standardized=T)
...
#METRICA
metrica_escolaridade <- measEq.syntax(configural.model=modelo_S, data=dt_MT_SP_AFC,
                         ordered=TRUE, estimator="WLSMV",
                         group="banco", return.fit=TRUE,
                         group.equal=c("thresholds","loadings"))
# summary(metrica_escolaridade, fit.measures=T, standardized=T)
...
#ESCALAR
escalar_escolaridade <- measEq.syntax(configural.model=modelo_S, data=dt_MT_SP_AFC,
                         ordered=TRUE, estimator="WLSMV",
                         group="banco", return.fit=TRUE,
                         group.equal=c("thresholds","loadings", 
                                       "intercepts"))
...
#THRESHOLDS 
thresholds_escolaridade <-measEq.syntax(configural.model=modelo_C, data=dt_MT_SP_AFC,
                           ordered=TRUE, estimator="WLSMV",
                           group="banco", return.fit=TRUE,
                           group.equal="thresholds")
# summary(thresholds_escolaridade, fit.measures=T, standardized=T)
...
#METRICA
metrica_escolaridade <- measEq.syntax(configural.model=modelo_C, data=dt_MT_SP_AFC,
                         ordered=TRUE, estimator="WLSMV",
                         group="banco", return.fit=TRUE,
                         group.equal=c("thresholds","loadings"))
# summary(metrica_escolaridade, fit.measures=T, standardized=T)
...
#ESCALAR
escalar_escolaridade <- measEq.syntax(configural.model=modelo_C, data=dt_MT_SP_AFC,
                         ordered=TRUE, estimator="WLSMV",
                         group="banco", return.fit=TRUE,
                         group.equal=c("thresholds","loadings", 
                                       "intercepts"))
...
# Diferenças de médias{.tabset .tabset-fade}
```{r include=FALSE}
...
R <- R[-c(9, 24)]
```
Precisamos decidir se vai computador os casos com NA. Se sim acrescentar "na.rm = TRUE" na hora de calcular os escores ou utilizar outro método pairwise.
...
## diferença de média masculino (1) x feminino (2) - 9º ano
```{r}
# selecionar somente os participante do 9º ano
dt_MT_2022_calculado <- dt_MT_SP %>% 
  filter(banco == "MT")
...
## diferença de média todos x EM
```{r}
# selecionar somente banco SP
dt_SP_2019_calculado <- dt_MT_SP %>% 
  filter(banco == "SP")

# juntar bancos
dt_MT_SP_todos_SP <- bind_rows(list(todos = dt_MT_SP, SP = dt_SP_2019_calculado), .id = "amostra")


# analisar diferença de média
jmv::ttestIS(
    formula = manual + natureza + tecnologia + seguranca + esportes + cie_naturais + cie_humanas + criacao + performance + exibicao + aconselhamento + educacao + saude + familia + altruismo + religiosidade + lideranca + organizacional + politica + vendas + rotinas + financas + R + I + A + S + E + C ~ amostra,
    data = dt_MT_SP_todos_SP,
    vars = vars(manual, natureza, tecnologia, seguranca, esportes, cie_naturais, cie_humanas, criacao, performance, exibicao, aconselhamento, educacao, saude, familia, altruismo, religiosidade, lideranca, organizacional, politica, vendas, rotinas, financas, R, I, A, S, E, C),
    welchs = TRUE,
    effectSize = TRUE,
    desc = TRUE)
...
# Normas amostra dividida por sexo - EM {.tabset .tabset-fade}
...
# # selecionar variáveis se interesse
# dt_MT_SP_SPSS <- dt_MT_SP %>%
#   select(
#     banco:Sexo,
#     manual:C
#   )
# 
# # salvar banco de dados em spss
# haven::write_sav(dt_MT_SP_SPSS, "dt_MT_SP_SPSS.sav")
```
